            ...
            <Pagination pageSize={pageItems}>
              {sortedTimes.map((p) => (
                <li key={p.id} className={`${styles.row} ${styles.item}`}>
                  <p className={styles.start}>{format(p.startTime, 'yyyy-MM-dd HH:mm:ss')}</p>
                  ...
                  {cost != undefined && p.endTime && (
                    <p className={styles.cost}>
                      {((differenceInSeconds(p.endTime, p.startTime) / 3600) * cost).toFixed(2)}€
                    </p>
                  )}
                  {hasActions ? <div className={styles.actions}>{renderActions?.(p)}</div> : null}
                </li>
              ))}
            </Pagination>
          </>
