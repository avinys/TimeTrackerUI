...
import styles from "../styles/summary.dateRangeSelector.module.css";
...
const DateRangeSelector: React.FC<DateRangeSelectorProps> = ({
	...
	return (
		<div className={styles.wrap}>
			{/* Range Type */}
			<select
				className={`${styles.input} ${styles.rangeType}`}
				value={rangeType}
			...
			{customFrom && customTo && new Date(customFrom) > new Date(customTo) && (
				<p className={styles.error}>End date cannot be earlier than start date.</p>
			)}
			...
			{rangeType === "weekly" && weeks.length > 0 && (
				<div className={styles.row}>
					<select className={styles.input} onChange={handleWeekChange} defaultValue="">
						<option value="" disabled>
							--select week--
						</option>
