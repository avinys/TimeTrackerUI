...
const DateRangeSelector: React.FC<DateRangeSelectorProps> = ({ value, onChange }) => {
    ...
    const month = value && ("month" in value || "day" in value) ? (value as any).month : null;
    const day = value && value.type === "daily" ? value.day : null;
    ...
    return (
        <div>
            {/* Range Type */}
            <select value={rangeType} onChange={(e) => handleRangeTypeChange(e.target.value as RangeType)}>
            ...
            {custom?.from && custom?.to && new Date(custom.from) > new Date(custom.to) && (
            ...
            {rangeType === "weekly" && weeks.length > 0 && (
                <select onChange={(e) => handleWeekChange(Number(e.target.value))}>
            ...
            {rangeType === "daily" && (
                <input
                    ...
                    value={
                        value?.type === "daily"
                            ? `${value.year}-${String(value.month + 1).padStart(2, "0")}-${String(value.day).padStart(
                                  2,
                                  "0"
                              )}`
                            : ""
